...
class PaymobPayment {
    ...
    private function getPaymentKey($orderId, $amount, $billingData, $paymentMethod = 'card') {
        ...
        // في إصدار الأحدث من API، لا نحتاج لإضافة source_data هنا
    ...
    public function processCallback($data) {
        ...
        } elseif (isset($data['obj']['order']['merchant_order_id'])) {
            $intentionId = $data['obj']['order']['merchant_order_id'];
        }
        ...
        $this->logInfo("Extracted IDs from callback", [
            'transaction_id' => $transactionId,
        ...
        if (empty($intentionId) && empty($orderId) && empty($transactionId)) {
        ...
        // البحث عن المعاملة باستخدام معرف المعاملة
        $transaction = null;
        ...
        if (!$transaction && !empty($intentionId)) {
            $stmt = $this->pdo->prepare("
                SELECT * FROM payment_transactions 
                WHERE intention_id = ? 
                ORDER BY created_at DESC 
            ...
            // استخراج معرف الاشتراك من معرف النية (إذا كان متاحًا)
            if (!$transaction && strpos($intentionId, '_') !== false) {
                $parts = explode('_', $intentionId);
        ...
        if (!$transaction) {
            $this->logError("No transaction found for transaction ID: $transactionId, intention ID: $intentionId or order ID: $orderId");
            
            // محاولة استخراج معرف الاشتراك من معرف النية
            $subscriptionId = null;
            if (!empty($intentionId) && strpos($intentionId, '_') !== false) {
                $parts = explode('_', $intentionId);
                if (count($parts) > 0 && is_numeric($parts[0])) {
                    $subscriptionId = $parts[0];
                }
            ...
            // إذا تم العثور على معرف الاشتراك، حاول إنشاء معاملة جديدة
            if (!empty($subscriptionId)) {
                $this->logInfo("Attempting to create a new transaction for subscription ID: $subscriptionId");
                ...
                try {
                    ...
                    if ($subscription) {
                        ...
                        $stmt = $this->pdo->prepare("
                            INSERT INTO payment_transactions 
                            (subscription_id, transaction_id, payment_method, amount, currency, status, 
                             paymob_order_id, paymob_transaction_id, paymob_response, created_at, updated_at) 
                            VALUES 
                            (?, ?, 'paymob', ?, ?, ?, ?, ?, ?, NOW(), NOW())
                        ");
                        $stmt->execute([
                        ...
                        $newTransactionId = $this->pdo->lastInsertId();
                        $this->logInfo("Created new transaction", [
                        ...
                        if ($success) {
                            $this->updateSubscriptionAfterPayment($subscriptionId);
                        }
                    ...
                    $this->logError("Error creating new transaction: " . $e->getMessage());
                }
            }
        ...
        // تحديث حالة المعاملة
        $status = $success ? 'completed' : 'failed';
        $this->updateTransactionStatus($transaction['id'], $status, json_encode($data), $transactionId);
     ...
     * @param string|null $paymobTransactionId Paymob transaction ID
     * @return bool
     */
    private function updateTransactionStatus($transactionId, $status, $responseData = null, $paymobTransactionId = null) {
        // تحديث المعاملة مع معرف المعاملة من باي موب إذا كان متاحًا
        if ($paymobTransactionId) {
            ...
            return $stmt->execute([$status, $paymobTransactionId, $responseData, $transactionId]);
        } else {
            $stmt = $this->pdo->prepare("
                UPDATE payment_transactions 
                SET status = ?, 
    ...
    private function createPaymentIntentionV1($subscriptionId, $amount, $billingData, $paymentMethod = 'card') {
        ...
        $intentionData = [
            "amount" => $amountCents, // المبلغ بالقروش
            "currency" => $this->currency,
