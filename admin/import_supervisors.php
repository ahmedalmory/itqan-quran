...
function importSupervisorsCsv($tempFile) {
    ...
    try {
        ...
        $checkUserStmt = $pdo->prepare("SELECT id FROM users WHERE name = ? AND phone = ?");
        ...
        $updateCircleStmt = $pdo->prepare("
            UPDATE study_circles SET supervisor_id = ? WHERE name = ?
        ");

        // تخزين المشرفين المضافة لمنع التكرار
        $addedSupervisors = [];
        ...
        while (($data = fgetcsv($handle)) !== FALSE) {
            ...
            $checkUserStmt->execute([$name, $mobile]);
            $supervisorId = $checkUserStmt->fetchColumn();
            
            // إذا لم يتم إضافة المشرف بعد
            if (!$supervisorId && !isset($addedSupervisors[$name.'_'.$mobile])) {
                // Add new supervisor
                $password = '1234567';
                ...
                $addedSupervisors[$name.'_'.$mobile] = true;
            }

            // Update study circle with supervisor
            if ($supervisorId) {
                $updateCircleStmt->execute([$supervisorId, $halaqaName]);
                $supervisorCount++;
                $currentBatch++;
            }
            ...
            if ($currentBatch >= $batchSize) {
                ...
                echo "<div style='background:#eee;padding:5px;margin:5px;'>تم معالجة $supervisorCount مشرف...</div>";
                ob_flush();
                flush();
            }
        }
        ...
        $success = sprintf("تم استيراد %d مشرف بنجاح", $supervisorCount);
